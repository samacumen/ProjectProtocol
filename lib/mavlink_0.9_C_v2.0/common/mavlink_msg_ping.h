...
#define MAVLINK_MSG_ID_PING_CRC 105
#define MAVLINK_MSG_ID_3_CRC 105
...
#define MAVLINK_MESSAGE_INFO_PING { \
	...
	"PING", \
	4, \
	{  { "time", NULL, MAVLINK_TYPE_UINT64_T, 0, 0, offsetof(mavlink_ping_t, time) }, \
...
{
	return mavlink_msg_ping_pack_chan(system_id, component_id, chan, msg, ping->seq, ping->target_system, ping->target_component, ping->time);
}
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_ping_send(chan, ping->seq, ping->target_system, ping->target_component, ping->time);
#else
 ...
 */
static inline void mavlink_msg_ping_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint32_t seq, uint8_t target_system, uint8_t target_component, uint64_t time)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
