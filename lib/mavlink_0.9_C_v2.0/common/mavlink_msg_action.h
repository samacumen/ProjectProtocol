...
#define MAVLINK_MSG_ID_ACTION_MIN_LEN 3
#define MAVLINK_MSG_ID_10_LEN 3
#define MAVLINK_MSG_ID_10_MIN_LEN 3

#define MAVLINK_MSG_ID_ACTION_CRC 60
#define MAVLINK_MSG_ID_10_CRC 60
...
{
	return mavlink_msg_action_pack_chan(system_id, component_id, chan, msg, action->target, action->target_component, action->action);
}
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_action_send(chan, action->target, action->target_component, action->action);
#else
 ...
 */
static inline void mavlink_msg_action_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t target, uint8_t target_component, uint8_t action)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
