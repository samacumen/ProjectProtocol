...
#define MAVLINK_MSG_ID_LOCAL_POSITION_MIN_LEN 32
						       ...
						       uint64_t usec, float x, float y, float z, float vx, float vy, float vz)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_LOCAL_POSITION_MIN_LEN, MAVLINK_MSG_ID_LOCAL_POSITION_LEN, MAVLINK_MSG_ID_LOCAL_POSITION_CRC);
}
						           ...
						           uint64_t usec,float x,float y,float z,float vx,float vy,float vz)
{
	...
	msg->msgid = MAVLINK_MSG_ID_LOCAL_POSITION;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_LOCAL_POSITION_MIN_LEN, MAVLINK_MSG_ID_LOCAL_POSITION_LEN, MAVLINK_MSG_ID_LOCAL_POSITION_CRC);
}
...
static inline void mavlink_msg_local_position_send(mavlink_channel_t chan, uint64_t usec, float x, float y, float z, float vx, float vy, float vz)
{
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_LOCAL_POSITION, buf, MAVLINK_MSG_ID_LOCAL_POSITION_MIN_LEN, MAVLINK_MSG_ID_LOCAL_POSITION_LEN, MAVLINK_MSG_ID_LOCAL_POSITION_CRC);
#else
	mavlink_local_position_t packet;
	packet.usec = usec;
	packet.x = x;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_LOCAL_POSITION, (const char *)&packet, MAVLINK_MSG_ID_LOCAL_POSITION_MIN_LEN, MAVLINK_MSG_ID_LOCAL_POSITION_LEN, MAVLINK_MSG_ID_LOCAL_POSITION_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_local_position_send_struct(mavlink_channel_t chan, const mavlink_local_position_t* local_position)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_local_position_send(chan, local_position->usec, local_position->x, local_position->y, local_position->z, local_position->vx, local_position->vy, local_position->vz);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_LOCAL_POSITION, (const char *)local_position, MAVLINK_MSG_ID_LOCAL_POSITION_MIN_LEN, MAVLINK_MSG_ID_LOCAL_POSITION_LEN, MAVLINK_MSG_ID_LOCAL_POSITION_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_LOCAL_POSITION_LEN? msg->len : MAVLINK_MSG_ID_LOCAL_POSITION_LEN;
        memset(local_position, 0, MAVLINK_MSG_ID_LOCAL_POSITION_LEN);
	memcpy(local_position, _MAV_PAYLOAD(msg), len);
#endif
}
