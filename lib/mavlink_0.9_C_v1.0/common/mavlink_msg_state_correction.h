...
#define MAVLINK_MSG_ID_STATE_CORRECTION_MIN_LEN 36
#define MAVLINK_MSG_ID_64_LEN 36
#define MAVLINK_MSG_ID_64_MIN_LEN 36
						       ...
						       float xErr, float yErr, float zErr, float rollErr, float pitchErr, float yawErr, float vxErr, float vyErr, float vzErr)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_STATE_CORRECTION_MIN_LEN, MAVLINK_MSG_ID_STATE_CORRECTION_LEN, MAVLINK_MSG_ID_STATE_CORRECTION_CRC);
}
						           ...
						           float xErr,float yErr,float zErr,float rollErr,float pitchErr,float yawErr,float vxErr,float vyErr,float vzErr)
{
	...
	msg->msgid = MAVLINK_MSG_ID_STATE_CORRECTION;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_STATE_CORRECTION_MIN_LEN, MAVLINK_MSG_ID_STATE_CORRECTION_LEN, MAVLINK_MSG_ID_STATE_CORRECTION_CRC);
}
...
static inline void mavlink_msg_state_correction_send(mavlink_channel_t chan, float xErr, float yErr, float zErr, float rollErr, float pitchErr, float yawErr, float vxErr, float vyErr, float vzErr)
{
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_STATE_CORRECTION, buf, MAVLINK_MSG_ID_STATE_CORRECTION_MIN_LEN, MAVLINK_MSG_ID_STATE_CORRECTION_LEN, MAVLINK_MSG_ID_STATE_CORRECTION_CRC);
#else
	mavlink_state_correction_t packet;
	packet.xErr = xErr;
	packet.yErr = yErr;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_STATE_CORRECTION, (const char *)&packet, MAVLINK_MSG_ID_STATE_CORRECTION_MIN_LEN, MAVLINK_MSG_ID_STATE_CORRECTION_LEN, MAVLINK_MSG_ID_STATE_CORRECTION_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_state_correction_send_struct(mavlink_channel_t chan, const mavlink_state_correction_t* state_correction)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_state_correction_send(chan, state_correction->xErr, state_correction->yErr, state_correction->zErr, state_correction->rollErr, state_correction->pitchErr, state_correction->yawErr, state_correction->vxErr, state_correction->vyErr, state_correction->vzErr);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_STATE_CORRECTION, (const char *)state_correction, MAVLINK_MSG_ID_STATE_CORRECTION_MIN_LEN, MAVLINK_MSG_ID_STATE_CORRECTION_LEN, MAVLINK_MSG_ID_STATE_CORRECTION_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_STATE_CORRECTION_LEN? msg->len : MAVLINK_MSG_ID_STATE_CORRECTION_LEN;
        memset(state_correction, 0, MAVLINK_MSG_ID_STATE_CORRECTION_LEN);
	memcpy(state_correction, _MAV_PAYLOAD(msg), len);
#endif
}
